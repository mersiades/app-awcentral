...
describe('Creating a new Chopper Character', () => {
  ...
  it('should create a Chopper and stop at BattleVehicleForm', () => {
    ...
    cy.get('div[data-testid="chopper-button"]').click();
    cy.contains('SELECT Chopper').click(); // SELECT CHOPPER
    ...
    const body = 'fat body';
    // Check form content
    cy.contains(`WHAT DOES ${chopperNameUC} LOOK LIKE?`).should('exist');

    // Check CharacterCreationStepper
    cy.get('div[data-testid="name-box"]').should('contain', NAME_TITLE).should('contain', chopperName);
    ...
    // ------------------------------------------ CharacterStatsForm ------------------------------------------ //
    // Check form content
    cy.contains(`WHAT ARE ${chopperNameUC}'S STRENGTHS AND WEAKNESSES`).should('exist');
    ...
    // Check form content
    cy.contains(`WHAT IS ${chopperNameUC}'S GEAR`).should('exist');
    ...
    // Check form functionality
    cy.get('@itemInput').type(chopperClothes);

    cy.get('@addButton').click();
    cy.get('@interimGearList').should('contain', chopperClothes);

    cy.contains(chopperWeapon1).click();
    cy.get('@addButton').click();
    cy.get('@interimGearList').should('contain', chopperWeapon1);

    cy.contains(chopperWeapon2).click();
    cy.get('@addButton').click();
    cy.get('@interimGearList').should('contain', chopperWeapon2);
    ...
    cy.get(`input[aria-label="option-${strength1Text}"]`).as('strength1');
    cy.get(`input[aria-label="option-${strength2Text}"]`).as('strength2');
    cy.get(`input[aria-label="option-${strength3Text}"]`).as('strength3');
    cy.get(`input[aria-label="option-${strength4Text}"]`).as('strength4');
    cy.get(`input[aria-label="option-${strength5Text}"]`).as('strength5');
    cy.get(`input[aria-label="option-${strength6Text}"]`).as('strength6');
    cy.get(`input[aria-label="option-${weakness1Text}"]`).as('weakness1');
    cy.get(`input[aria-label="option-${weakness2Text}"]`).as('weakness2');
    cy.get(`input[aria-label="option-${weakness3Text}"]`).as('weakness3');
    cy.get(`input[aria-label="option-${weakness4Text}"]`).as('weakness4');
    cy.get(`input[aria-label="option-${weakness5Text}"]`).as('weakness5');
    cy.get(`div[data-testid="size-box"]`).as('sizeBox');
    cy.get(`h2[aria-label="harm-value"]`).as('harmValue');
    cy.get(`h2[aria-label="armor-value"]`).as('armorValue');
    cy.get(`div[data-testid="tags-tags-box"]`).as('tagsBox');
    ...
    cy.get('@sizeBox').should('contain', 'MEDIUM');

    cy.contains(strength2Text).click();
    cy.get('@strength2').should('be.checked');
    cy.get('@sizeBox').should('contain', 'MEDIUM');
    cy.get('@harmValue').should('include.text', '3');
    ...
    cy.get('@sizeBox').should('contain', 'MEDIUM');
    cy.get('@harmValue').should('include.text', '2');

    cy.contains(strength3Text).click();
    cy.get('@strength3').should('be.checked');
    cy.get('@sizeBox').should('contain', 'MEDIUM');
    cy.get('@armorValue').should('include.text', '2');
    cy.get('@tagsBox').should('contain', 'savage');

    cy.contains(strength1Text).click();
    cy.get('@strength1').should('not.be.checked');
    cy.get('@sizeBox').should('contain', 'SMALL');
    cy.get('@armorValue').should('include.text', '2');
    cy.get('@tagsBox').should('contain', 'savage');
    ...
    cy.get('@sizeBox').should('contain', 'SMALL');
    cy.get('@armorValue').should('include.text', '2');
    cy.get('@tagsBox').should('not.contain', 'savage');
    ...
    cy.get('@sizeBox').should('contain', 'SMALL');
    cy.get('@armorValue').should('include.text', '1');
    cy.get('@tagsBox').should('not.contain', 'savage').should('contain', 'mobile');

    cy.contains(strength4Text).click();
    cy.get('@strength4').should('not.be.checked');
    cy.get('@sizeBox').should('contain', 'SMALL');
    cy.get('@armorValue').should('include.text', '1');
    cy.get('@tagsBox').should('contain', 'savage').should('contain', 'mobile');

    cy.contains(strength6Text).click();
    cy.get('@strength6').should('be.checked');
    cy.get('@sizeBox').should('contain', 'SMALL');
    cy.get('@armorValue').should('include.text', '1');
    cy.get('@tagsBox').should('contain', 'savage').should('contain', 'mobile').should('contain', 'rich');
