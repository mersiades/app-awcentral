...
self.addEventListener('message', async function (event) {
  ...
  switch (event.data) {
    ...
    case 'MOCK_ACTIVATE': {
      sharedClientId = clientId; // CUSTOM MOD
...
self.addEventListener('fetch', async function (event) {
  const { clientId: newClientId, request } = event;
  const clientId = sharedClientId || newClientId; // CUSTOM MOD
