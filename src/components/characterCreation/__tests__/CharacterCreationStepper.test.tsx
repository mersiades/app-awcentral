...
describe('Rendering CharacterCreationStepper', () => {
  ...
  describe('with Angel setting name', () => {
    ...
    test('should render playbook, name title, looks title, stats title and gear title', () => {
      ...
      expect(
        screen.getByText(decapitalize(mockAngel_fresh.playbook))
      ).toBeInTheDocument();
      ...
      expect(
        screen.queryByText(decapitalize(MOVES_TITLE))
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(decapitalize(uniqueType as UniqueTypes))
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe('with Angel on Moves screen', () => {
    ...
    test('should render stepper with gear, angel kit, moves and vehicles (title only)', () => {
      ...
      expect(screen.getByText(GEAR_TITLE)).toBeInTheDocument();
      expect(
        screen.getByText(mockAngel_readyToAddMoves.gear[0])
      ).toBeInTheDocument();
      expect(
        screen.getByText(mockAngel_readyToAddMoves.gear[1])
      ).toBeInTheDocument();
      expect(
        screen.getByText(decapitalize(uniqueType as UniqueTypes))
      ).toBeInTheDocument();
      expect(screen.getByText(MOVES_TITLE)).toBeInTheDocument();
      expect(
        screen.getByText(
          decapitalize(mockAngel_readyToAddMoves.characterMoves[0].name)
        )
      ).toBeInTheDocument();
