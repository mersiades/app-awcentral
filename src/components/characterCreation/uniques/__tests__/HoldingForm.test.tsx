...
import { DECREASED_BY_IMPROVEMENT_TEXT, INCREASED_BY_IMPROVEMENT_TEXT } from '../../../../config/constants';
...
describe('Rendering HoldingForm', () => {
  ...
  describe('with a fresh Holding', () => {
    ...
    test('should render HoldingForm in intial state', () => {
      ...
      screen.getByRole('heading', { name: `${mockCharacter2.name?.toUpperCase()}'S HOLDING` });
      const holdingSize = screen.getByRole('heading', { name: 'holding size-value' });
      expect(holdingSize.textContent).toEqual(mockHolding_noSelectionsMade.holdingSize);
      const gangSize = screen.getByRole('heading', { name: 'gang size-value' });
      expect(gangSize.textContent).toEqual(mockHolding_noSelectionsMade.gangSize);
      const surplusValue = screen.getByRole('heading', { name: 'surplus-value' });
      expect(surplusValue.textContent).toContain(mockHolding_noSelectionsMade.surplus.toString());
      const defenseBonusValue = screen.getByRole('heading', { name: 'defense bonus-value' });
      expect(defenseBonusValue.textContent).toContain(mockHolding_noSelectionsMade.gangDefenseArmorBonus.toString());
      const harmValue = screen.getByRole('heading', { name: 'harm-value' });
      expect(harmValue.textContent).toEqual(mockHolding_noSelectionsMade.gangHarm.toString());
      const armorValue = screen.getByRole('heading', { name: 'armor-value' });
      expect(armorValue.textContent).toEqual(mockHolding_noSelectionsMade.gangArmor.toString());
  ...
  describe('with a Holding with three improvements', () => {
    beforeEach(async () => {
      renderWithRouter(<HoldingForm />, `/character-creation/${mockGame5.id}`, {
        isAuthenticated: true,
        apolloMocks: [mockPlayBookCreatorQueryHardHolder],
        injectedGame: generateGame(mockPlaybookUniqueHardHolder_with3Improvements),
        injectedUserId: mockKeycloakUserInfo1.sub,
        cache,
      });
    ...
    test('should render with adjusted strength and weakness counts', () => {
      ...
      expect(screen.getByText(DECREASED_BY_IMPROVEMENT_TEXT)).toBeInTheDocument();
    });
  });
});
