...
import ScriptChangeDialog, { SCRIPT_CHANGE_COMMENT_INPUT_ID } from '../ScriptChangeDialog';
...
describe('Rendering ScriptChangeDialog', () => {
  ...
  test('should display all the Script Change content', () => {
    ...
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_PAUSE_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_RESUME_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_REPLAY_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_FAST_FORWARD_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_REWIND_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: SCRIPT_CHANGE_FRAME_TITLE })).toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: SCRIPT_CHANGE_COMMENT_INPUT_ID })).toBeInTheDocument();
    expect(screen.getByRole('link', { name: SCRIPT_CHANGE_ATTRIBUTION_TEXT_4 })).toBeInTheDocument();
    ...
    expect(screen.getByText(SCRIPT_CHANGE_FAST_FORWARD_CONTENT)).toBeInTheDocument();
  ...
  test('should type a comment, click an option, and close the dialog', () => {
    ...
    const commentInput = screen.getByRole('textbox', { name: SCRIPT_CHANGE_COMMENT_INPUT_ID }) as HTMLInputElement;
