...
const FirstSessionDialog = () => {
  ...
  // ------------------------------------------------------ graphQL -------------------------------------------------------- //
  const [closeFirstSession, { loading: closingFirstSession }] = useMutation<CloseFirstSessionData, CloseFirstSessionVars>(
    CLOSE_FIRST_SESSION
  );
    ...
    if (!!gameId && !closingFirstSession) {
      try {
        await closeFirstSession({
          variables: { gameId },
          optimisticResponse: getCloseFirstSessionOR(gameId) as CloseFirstSessionData,
        });
      } catch (error) {
        console.error(error);
      }
